...
def render(
    *,
    ...
    if gcode:
        ...
        lines.append(
            "- Skipped (no slicer run) -- filament/time metrics unavailable; "
            "checks below cover geometry only."
        )
    lines += [
        "",
        "## Printability Checks",
    ...
    lines += [
        ...
        "- Slicer resolved-settings hash: "
        f"{(slice_result.resolved_settings_sha256 or 'n/a') if slice_result else 'n/a'}",
