"""Integration tests: build each real example (bracket, benchy) and slice it
with each available backend. Skipped per-backend if that backend's binary
isn't installed (see printlab.slicing.detect_all). Requires CadQuery to
build the part, so this file only runs in the CI heavy lane, never the fast
lane.
"""
...
@pytest.fixture(scope="module", params=["bracket", "benchy"])
def built_example(request, tmp_path_factory):
    example_name = request.param
    config = pipeline.load_part_config(EXAMPLES_ROOT / example_name)
